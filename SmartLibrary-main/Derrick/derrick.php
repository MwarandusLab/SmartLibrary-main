...
<body>
    ...
    function getBookStatus($conn, $bookId)
    {
        ...
        if ($status_derrick === 'Borrow' && $status_linah === 'Borrow' && $status_kheri === 'Borrow') {
            return 'Borrow';
        } elseif ($status_derrick === 'Borrow' && $status_linah === 'Booked' && $status_kheri === 'Borrow') {
            // If any table has the status "Booked," return "Booked"
            return 'Not Available';
        } elseif ($status_derrick === 'Borrow' && $status_linah === 'Borrow' && $status_kheri === 'Booked') {
            // If any table has the status "Booked," return "Booked"
            return 'Not Available';
        } elseif ($status_derrick === 'Booked' && $status_linah === 'Borrow' && $status_kheri === 'Borrow') {
            // If any table has the status "Booked," return "Booked"
            return 'Booked';
        }
    }
    ...
    <main>
        <h3 class="User">Welcome Back Derrick</h3>
        <h2>PHYSICS BOOKS</h2>
    ...
    <script>
        function buttonClicked(bookId) {
            ...
            if (button && button.textContent.trim() === 'Borrow') {
                ...
                xhttp.open("POST", "physics_status_1.php", true);
        ...
        function buttonClicked_2(bookId) {
            ...
            if (button && button.textContent.trim() === 'Borrow') {
                ...
                xhttp.open("POST", "engineering_status_1.php", true);
                xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                xhttp.send("bookId=" + bookId);
            } else {
                console.log("Button action not allowed. Current status is not 'Borrow'.");
        ...
        function buttonClicked_3(bookId) {
            ...
            if (button && button.textContent.trim() === 'Borrow') {
                ...
                xhttp.open("POST", "mathematics_status_1.php", true);
                xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                xhttp.send("bookId=" + bookId);
            } else {
                console.log("Button action not allowed. Current status is not 'Borrow'.");
